...
class TreeVisibility {
  ...
  TreeVisibility clone() => TreeVisibility._(
        mode: mode,
  ...
  static TreeWidget fromRootNode({
    ...
    double? itemExtent,
      ...
      TreeWidget(
        ...
        treeData: TreeDataVisible.fromTreeNode(
          root: rootNode,
      ...
      itemBuilder: (c, i) {
        ...
        return node.builder(
          ...
          TreeBuilderDetails(
            ...
            index: data.index,
            last: data.last,
            // TODO isExpanded should be stored in visitor data
            isExpanded: data.isExpanded(treeData.treeVisibility),
          ),
        );
      },
    );
...
class TreeVisitorData {
  ...
  /// Skip data only available when constructed via [TreeVisitor.complete]
  ...
  /// Only available when constructed via [TreeVisitor.complete]
...
abstract class TreeVisitor {
  ...
  ///
  /// To apply visibility to this result, see [TreeVisitor.visibleFromComplete]
  static List<TreeVisitorData> complete(TreeNodeAbst rootNode) {
  ...
  /// Takes in the flattened complete list generated by [TreeVisitor.complete] and applies
  /// [visibility] to it.
  static Iterable<TreeVisitorData> visibleFromComplete(
    List<TreeVisitorData> flattened,
    TreeVisibility visibility,
  ) sync* {
    assert(
      flattened.isEmpty || flattened.firstWhereOrNull((e) => e.skip == null) == null,
      "all elements of <flattened> must have <skip> set. See TreeVisitor.flatten",
    );

    for (int i = 0; i < flattened.length; i++) {
      final item = flattened[i];
      yield item;
      if (!visibility.isExpanded(item.node.id)) {
        i += item.skip!;
      }
    }
  }
...
abstract class TreeDataAbst {
  final List<TreeVisitorData> list;
...
class TreeDataVisible extends TreeDataAbst {
  TreeDataVisible._({required super.list, required super.treeVisibility});

  static TreeDataVisible fromTreeNode({
    required TreeNodeAbst root,
    required TreeVisibility treeVisibility,
  }) {
    final _visibility = treeVisibility.clone();
    return TreeDataVisible._(
      list: TreeVisitor.visible(root, _visibility).toList(),
      treeVisibility: _visibility,
    );
  }
}

class TreeDataComplete extends TreeDataAbst {
  TreeDataComplete._({required super.list, required super.treeVisibility});

  static TreeDataComplete fromTreeNode({
    required TreeNodeAbst root,
  }) {
    return TreeDataComplete._(
      list: TreeVisitor.complete(root).toList(),
      treeVisibility: TreeVisibility.allExpanded(),
    );
  }

  TreeDataVisible applyVisibility(TreeVisibility treeVisibility) {
    final visibility = treeVisibility.clone();
    return TreeDataVisible._(
      list: TreeVisitor.visibleFromComplete(list, visibility).toList(),
      treeVisibility: visibility,
    );
  }
}
